...
filename = f'genomes_and_entropies__lgen_{l_gen}__unif_{",".join([str(l) for l in ls_motif_in])}' \
           + f'__L1_{L1_target_in}__L2_{L2_target_in}.txt'
...
genomes, _ = \
    ...
    L1_target=L1_target_out, L2_target=L2_target_out, N_steps=int(1e5), tau=1e-5)
...
write_genomes_and_entropies_to_file(genomes_rel, entropies_rel, l_gen=l_gen, ls_motif=ls_motif_all, \
    ...
    filename=f'genomes_and_entropies__lgen_{l_gen}__unif_{",".join([str(l) for l in ls_motif__immut])}__' \
             f'weaknonunif_{",".join([str(l) for l in ls_motif__mut])}__L1_{L1_target_out}__L2_{L2_target_out}.txt')
