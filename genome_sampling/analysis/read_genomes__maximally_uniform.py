...
ls_motif__immut = np.array([1,2,3,4])
...
entropies_hist = {}
for entropies_loc in entropies[:,ls_motif__immut-1]:
...
genome_indices = np.argwhere(np.all(np.round(entropies[:,ls_motif__immut-1],10) == entropy_of_interest, axis=1))[:,0]
